...
function selectChannelModal() {
  const background = document.createElement('div');
  background.style.cssText = 'display:none;opacity:0;background-color:rgb(0,0,0,0.7);position:absolute;top:0;left:0;width:100%;height:100%;transition: top 2s, opacity 2s;z-index:2147483646';
  background.id = "urbit-visor-extension-modal-wrapper";
  const foreground = document.createElement('div');
  foreground.id = "urbit-visor-channel-select-modal";
  foreground.style.cssText = 'color:white;background-color:black;border:3px solid red;position:absolute;top:20px;right:50px;width:100%;height:500px;padding:0.5rem;text-align:center;font-weight:700;z-index:2147483647';
  const logo = document.createElement('div');
  logo.style.cssText = "margin: auto";
  logo.innerHTML = `
  <svg width="68" height="68" viewBox="0 0 450 450" fill="none" xmlns="http://www.w3.org/2000/svg">
